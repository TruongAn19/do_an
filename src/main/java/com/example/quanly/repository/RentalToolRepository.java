...
public interface RentalToolRepository extends JpaRepository<RentalTool, Long> {
    ...
    @Query("SELECT rkt, SUM(rt.quantity) FROM RentalTool rt JOIN Racket rkt ON rt.racketId = rkt.id WHERE rt.type = 'DAILY' AND rt.rentalDate BETWEEN :startDate AND :endDate AND rt.status = 'CONFIRMED' AND rkt.product.id = :courtId GROUP BY rkt ORDER BY SUM(rt.quantity) DESC")
    ...
    @Query("SELECT COUNT(rt) FROM RentalTool rt WHERE rt.rentalDate BETWEEN :startDate AND :endDate AND rt.status = :status")
    int countByRentalDateBetweenAndStatus(@Param("startDate") LocalDate startDate,
                                          @Param("endDate") LocalDate endDate,
                                          @Param("status") RentalToolStatus status);


    // Tính tổng doanh thu trong khoảng thời gian
    @Query("SELECT SUM(rt.rentalPrice) FROM RentalTool rt WHERE rt.rentalDate BETWEEN :startDate AND :endDate AND rt.status = :status")
    Double sumRevenueByRentalDateBetweenAndStatus(@Param("startDate") LocalDate startDate,
                                                  @Param("endDate") LocalDate endDate,
                                                  @Param("status") RentalToolStatus status);
