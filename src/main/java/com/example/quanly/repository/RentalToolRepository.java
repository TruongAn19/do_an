...
public interface RentalToolRepository extends JpaRepository<RentalTool, Long> {
    ...
    @Query("SELECT COUNT(r) FROM RentalTool r WHERE r.rentalDate BETWEEN :startDate AND :endDate AND r.status IN (com.example.quanly.domain.RentalToolStatus.PAID, com.example.quanly.domain.RentalToolStatus.COMPLETED) AND (:courtId IS NULL OR r.productId = :courtId)")
    int countDailyRentalByCourtAndDateRange(@Param("courtId") Long courtId, @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    @Query("SELECT COUNT(r) FROM RentalTool r WHERE r.rentalDate BETWEEN :startDate AND :endDate AND r.status IN (com.example.quanly.domain.RentalToolStatus.PAID, com.example.quanly.domain.RentalToolStatus.COMPLETED) AND (:courtId IS NULL OR r.productId = :courtId) and r.id = :equipmentId")
    ...
    @Query("SELECT COALESCE(SUM(r.rentalPrice), 0) FROM RentalTool r WHERE  r.rentalDate BETWEEN :startDate AND :endDate AND r.status IN (com.example.quanly.domain.RentalToolStatus.PAID, com.example.quanly.domain.RentalToolStatus.COMPLETED) AND (:courtId IS NULL OR r.productId = :courtId)")
    double sumDailyRevenueByCourtAndDateRange(@Param("courtId") Long courtId, @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    @Query("SELECT rkt, SUM(rt.quantity), SUM(rt.rentalPrice) FROM RentalTool rt JOIN Equipment rkt ON rt.equipmentId = rkt.id WHERE rt.rentalDate BETWEEN :startDate AND :endDate AND rt.status IN (com.example.quanly.domain.RentalToolStatus.PAID, com.example.quanly.domain.RentalToolStatus.COMPLETED) AND (:courtId IS NULL OR rkt.product.id = :courtId) GROUP BY rkt ORDER BY SUM(rt.quantity) DESC")
