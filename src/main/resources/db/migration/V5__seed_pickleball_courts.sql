...
-- Bổ sung 20 địa điểm pickleball (product id 10004-10023).
...
VALUES
(10004, 'Saigon Smash Pickleball', 110000,
...
(10005, 'Thảo Điền Pickleball Hub', 160000,
...
(10006, 'Phú Nhuận Paddle Club', 125000,
 'https://5.imimg.com/data5/SELLER/Default/2022/6/VM/UI/OB/11336583/indiana-sports-outdoor-tennis-court-flooring-500x500.jpg',
...
(10007, 'Tân Bình Ace Arena', 140000,
 'https://5.imimg.com/data5/SELLER/Default/2022/6/VM/UI/OB/11336583/indiana-sports-outdoor-tennis-court-flooring-500x500.jpg',
...
(10008, 'Riverside Pickleball Bình Thạnh', 130000,
...
(10009, 'Hà Nội Capital Pickleball', 150000,
...
(10010, 'West Lake Paddle House', 170000,
...
(10011, 'Long Biên Pickleball Center', 115000,
...
(10012, 'Mỹ Đình Pro Pickleball', 155000,
...
(10013, 'Đà Nẵng Ocean Pickleball', 120000,
 'https://www.meanwell.com/PBM/tw/img/portfolio/JR09-large.jpg',
...
(10014, 'Han River Paddle Club', 135000,
 'https://images.unsplash.com/photo-1761644273884-83839f8f22e5?auto=format&fit=crop&w=1200&q=80',
...
(10015, 'Nha Trang Sun Court', 125000,
...
(10016, 'Cần Thơ Mekong Pickleball', 105000,
...
(10017, 'Biên Hòa Green Paddle', 110000,
...
(10018, 'Bình Dương Champion Court', 130000,
...
(10019, 'Vũng Tàu Seaside Pickleball', 135000,
...
(10020, 'Huế Imperial Paddle', 100000,
...
(10021, 'Hải Phòng Port City Pickleball', 115000,
...
(10022, 'Hạ Long Bay Paddle Club', 145000,
...
(10023, 'Đà Lạt Highland Pickleball', 120000,
...
JOIN `available_time` t
  ON (p.`id` BETWEEN 10004 AND 10008  AND t.`id` BETWEEN 1 AND 18)
  OR (p.`id` BETWEEN 10009 AND 10013 AND t.`id` BETWEEN 2 AND 18)
  OR (p.`id` BETWEEN 10014 AND 10018 AND t.`id` BETWEEN 1 AND 17)
  OR (p.`id` BETWEEN 10019 AND 10023 AND t.`id` BETWEEN 3 AND 18)
WHERE p.`id` BETWEEN 10004 AND 10023;

-- Tạo sân con theo products.quantity. Mỗi product dành sẵn một dải 6 id.
INSERT INTO `sub_courts` (`id`, `name`, `product_id`)
SELECT 100013 + ((p.`id` - 10004) * 6) + (n.`id` - 1),
       CONCAT('Sân số ', n.`id`),
       p.`id`
FROM `products` p
...
WHERE sc.`product_id` BETWEEN 10004 AND 10023;
